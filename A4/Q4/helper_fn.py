...
def plot_a2c_curves(actor_losses,critic_losses,episode_rewards,algo='a2c'):
    plt.figure(figsize=(12, 6))
    plt.subplot(3, 1, 1)
    plt.plot(actor_losses, label="Actor Loss", alpha=0.8)
    
    plt.title("A2C Losses")
    ...
    plt.ylabel("Loss")
    plt.legend()
    # plt.plot(total_losses, label="Total Loss", alpha=0.6)
    plt.subplot(3, 1, 3)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'plots/{algo}_rewards.png')
    # plt.show()


def plot_rewards(a2c_path="rewards/a2c_rewards.npy", dqn_path="rewards/dqn_rewards.npy"):
    a2c_rewards = np.load(a2c_path)
    dqn_rewards = np.load(dqn_path)

    plt.figure(figsize=(10, 5))
    plt.plot(a2c_rewards, label="A2C Rewards")
    plt.plot(dqn_rewards, label="DQN Rewards")
    
    plt.title("A2C vs DQN Reward Comparison")
    plt.xlabel("Episode")
    plt.ylabel("Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('plots/rewards_cmp.png')
    plt.show()
