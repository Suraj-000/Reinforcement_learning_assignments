...
class RewardShapedDiscretePortfolioOptEnv(gym.Env): 
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.action_space = spaces.Box(low=np.array([-1*self.lot_size for l in range(self.num_assets)]),high=np.array([1*self.lot_size for l in range(self.num_assets)]),
                          dtype=np.int32)
        self.wealth_history=[]     
        self.prev_wealth=self.initial_cash
        self.seed()
        self.reset()
    ...
    def _RESET(self):
        ...
        self.holdings = np.zeros(self.num_assets)
        self.cash = copy(self.initial_cash)
        self.wealth_history=[self.initial_cash]
        self.prev_wealth=self.initial_cash
        self.state = np.hstack([
            self.initial_cash,
            self.asset_prices[:, self.step_count],
    ...
    def _STEP(self, action):
        ...
        cur_wealth=np.dot(asset_prices,self.holdings)+self.cash
        self.prev_wealth=self.wealth_history[-1]
        self.wealth_history.append(cur_wealth)
        reward=cur_wealth+self.prev_wealth
        self.step_count+=1
        # Return total portfolio value at the end of the horizon as reward
        if self.step_count>= self.step_limit: 
            done=True
