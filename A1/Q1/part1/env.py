...
class FootballSkillsEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, grid_size = 20, degrade_pitch=False, degrade_mode="sigmoid"):
        ...
        self.start_pos = (0, 10)
        # goal positions are [19,9],[19,10],[19,11] for grid_size=20
        ...
        # State is (x, y, has_shot) where has_shot is 0 or 1 total 800 states for grid_size=20
    ...
    def _get_reward(self, ball_pos, action, player_pos_at_shot):
        '''
        inputs: ball_pos: (x,y) position of the ball after action
                action: action taken (0-6)
                player_pos_at_shot: (x,y) position of player when shot was taken
        output: reward (int)
                '''
