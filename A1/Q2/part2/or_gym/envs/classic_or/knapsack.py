...
class OnlineKnapsackEnv2(BoundedKnapsackEnv):
    def __init__(self, *args, **kwargs):
        BoundedKnapsackEnv.__init__(self)
        assign_env_config(self, kwargs)
        self.action_space = spaces.Discrete(2)
        ...
        # Neutral names instead of cycle-related names
        self.item_order = [0, 1]  # Fixed deterministic item sequence
        self.item_index = 0       # Index to track current item in the sequence

        self.state = self.reset()
        self._max_reward = 600

    def _update_state(self):
        # Deterministic iteration through items, no cycle wording
        self.current_item = self.item_order[self.item_index]
        self.item_index = (self.item_index + 1) % len(self.item_order)
    ...
    def _RESET(self):
        ...
        self.current_weight = 0
        self.step_counter = 0
        self.item_index = 0  # Reset iteration index here
        self._update_state()
        return self.state
