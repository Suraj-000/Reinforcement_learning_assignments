...
class PolicyIterationPortfolio:
    def __init__(self, env, gamma=1.0,transaction_cost=1,cash_limit=100, epsilon=1e-2, eval_iterations=1000):
        self.env = env
        self.gamma = gamma
    ...
    def run_policy_iteration(self, max_iterations=1000):
        ...
        for i in range(max_iterations):
            deltas=self.policy_evaluation()
            delta_history.extend(deltas)
        ...
        return self.policy, self.value_function,wealth_history,tra,delta_history
...
if __name__=="__main__":
    ...
    '''
    QUESTION 1 
    Change the variance of price to 1.0 by providing keyword argument ”variance” set to 1.0 during environment
    initialization. Does the policy iteration algorithm converge to the optimal solution in this setting? Plot the
    maximum value difference vs iteration count for policy iteration. Assume convergence threshold of 10−2
    for this problem. You may limit the maximum iterations to 1000 for reporting this result.
    '''

    env = DiscretePortfolioOptEnv(varience=1.0)
    print(f' Asset prices={env.asset_prices} discount factor={0.95}')
    print('Starting Policy iteration training')
    P=PolicyIterationPortfolio(env,0.95,transaction_cost)
    policy,value_function,wealth_history,tra,delta_history=P.run_policy_iteration()
    ...
    plt.ylabel("Maximum Value Difference (Δ)")
    plt.title("Policy Iteration: Max Value Difference vs Iteration Count")
    plt.grid(True)
    plt.show()